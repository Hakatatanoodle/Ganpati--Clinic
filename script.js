...
const logoutModal=$("logoutModal");function openLogout(){logoutModal.classList.remove("hide");setTimeout(()=>$("logoutNo").focus(),0)}function closeLogout(){logoutModal.classList.add("hide")}$("logout").onclick=openLogout;$("sideLogout").onclick=openLogout;$("logoutNo").onclick=closeLogout;$("logoutYes").onclick=async()=>{closeLogout();await signOut(auth)};logoutModal.addEventListener("click",e=>{if(e.target===logoutModal)closeLogout()});document.addEventListener("keydown",e=>{if(e.key==="Escape"&&!logoutModal.classList.contains("hide"))closeLogout();if((e.altKey&&e.key==="ArrowLeft")||(e.key==="ArrowLeft"&&!e.target.matches("input,textarea,select,button"))){const b=$("prevSection");if(b&&!$("formCard").classList.contains("hide")){e.preventDefault();b.click()}}});$("clear").onclick=reset;$("saveTop").onclick=()=>$("patientForm").requestSubmit();$("cancel").onclick=reset;$("search").oninput=render;
function showSection(id){const sections=["dashboardCard","formCard","recordsCard","reportsCard","smsBroadcastCard","usersCard","backupCard","aboutCard"];sections.forEach(x=>$(x).classList.toggle("hide",x!==id));$("formActions").classList.toggle("hide",id!=="formCard");const navs=[...document.querySelectorAll(".nav[data-target]")];const activeNav=id==="formCard"?navs.find(b=>b.dataset.target==="formCard"):navs.find(b=>b.dataset.target===id);navs.forEach(b=>b.classList.toggle("active",b===activeNav));if(id==="recordsCard")render();const titles={dashboardCard:"Dashboard",formCard:"Add / Edit Patient Record",recordsCard:"Patient Records",reportsCard:"Reports",smsBroadcastCard:"SMS Broadcast",usersCard:"Users",backupCard:"Backup",aboutCard:"About Us"};$("pageTitle").textContent=titles[id]||"Dashboard";$("breadcrumb").textContent=id==="dashboardCard"?"Dashboard":`Dashboard › ${titles[id]||""}`;window.scrollTo({top:0,behavior:"smooth"})}
...
function updateSmsCount(){const n=($('smsMessage')?.value||"").length;$("smsCount").textContent=`${n} / 160`}
...
patientForm.addEventListener("focusin",e=>{const fields=getFieldOrder();const i=fields.indexOf(e.target);if(i>=0)lastFieldIndex=i});
