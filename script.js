...
function updateSmsCount(){const ta=$('smsMessage');if(!ta)return;const text=ta.value.trim();const words=text?text.split(/\s+/):[];if(words.length>1000){ta.value=words.slice(0,1000).join(" ");}const n=ta.value.trim()?ta.value.trim().split(/\s+/).length:0;$("smsCount").textContent=`${n} / 1000 words`}
...
$("prevSection").onclick=goPreviousSection;document.querySelectorAll(".nav[data-target]").forEach(b=>b.onclick=()=>showSection(b.dataset.target));document.querySelectorAll("[data-open]").forEach(b=>b.onclick=()=>showSection(b.dataset.open));
